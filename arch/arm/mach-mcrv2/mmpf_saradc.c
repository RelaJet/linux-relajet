...
{  
    ...
    if (HIGH_BYTE(data) & TP_ADC_DATA_VALID) {
       ...
       RTNA_DBG_Str3("\r\n");
       */
       /*applied compensation formula for Aux 2&3 output value*/       
       /*By default, AUX1 is used to measure current Battery usage, for protect circuit reason, 
                     external voltage-drop circuit is added, 
                     so Aux1 is also need compensation but not the same for Aux 2&3.                                     
       */
       printk(KERN_INFO"*********************** grp: %d\r\n",  (int)grp);
       *value = data & 0x3FF;      
       if (grp != 1) {
           if (EcoVer == 0) 
	   {
