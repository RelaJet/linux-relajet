...
//------------------------------------------------------------------------------
//  Function    : MMPF_RTC_ResetIfInvalid
//------------------------------------------------------------------------------
/** @brief Resets the RTC to 0 if its valid flag is not set.
    @return It reports the status of the operation.
*/
static MMP_ERR MMPF_RTC_ResetIfInvalid(void)
{
    if(!MMPF_RTC_IsValid()) {
        MMP_ERR res;

        printk(KERN_WARNING "[mmpf_rtc] Warning: RTC contents invalid. Resetting to 0.\n");
        res = MMPF_RTC_SetTimeInSec(0);
        if(res != MMP_ERR_NONE) {
            printk(KERN_ERR "[mmpf_rtc] ERROR: Failed to reset RTC: status=%d\n", res);
			return res;
        }
        else {
            if(MMPF_RTC_IsValid()) {
                printk(KERN_INFO "[mmpf_rtc] RTC reset successfully verified.\n");
            }
            else {
                printk(KERN_ERR "[mmpf_rtc] Warning: Failed to verify RTC reset.\n");
				return MMP_SYSTEM_ERR_HW;
            }
        }
    }
    else {
        printk(KERN_INFO "[mmpf_rtc] RTC contents valid\n");
    }
	return MMP_ERR_NONE;
}
...
MMP_ERR MMPF_RTC_Initialize(void)
{
...
#if 0	// Note: Disabled because this function didn't used to be called at all
	...
	return MMPF_RTC_ResetIfInvalid();
}
...
MMP_ERR	MMPF_RTC_SetTimeInSec(MMP_ULONG64 secs)
{
    ...
    //Retry if set RTC failed
    if (MMPF_RTC_SetValid()) {
      MMPF_RTC_SetTimeInSec(secs);;
    }
    return MMP_ERR_NONE;
}
EXPORT_SYMBOL(MMPF_RTC_SetTimeInSec);
...
EXPORT_SYMBOL(MMPF_RTC_GetTime_InSeconds);
