...
static void pmc_pll_mode(struct clk *clk, int is_on)
{
...
AUDIO_IPC_MODE audio_ipc_mode = AUDIO_IPC_EN ;
#else
AUDIO_IPC_MODE audio_ipc_mode = AUDIO_IPC_NONE ;
#endif
...
extern AUDIO_IPC_MODE audio_ipc_mode;
  ...
  if(audio_ipc_mode==AUDIO_IPC_EN) {
    if( !strcmp(clk->name,"aud_clk" ) || !strcmp(clk->name,"adc_clk") || !strcmp(clk->name,"dac_clk") ) {
#ifdef CLK_DEBUG
      pr_info("Try %s %s ,but is remote-cpu controlled\n",clk->name,is_on?"on":"off") ;      
#endif
      return ;  
    }  
  }
	...
	else
	{
		if(clk->type==CLK_TYPE_CLK_CTL1)
		{
			pGBL->GBL_CLK_DIS[0] |=clk->pmc_mask;
		}
		if(clk->type==CLK_TYPE_CLK_CTL2)
		{
			pGBL->GBL_CLK_DIS[1] |=clk->pmc_mask;
		}
...
static struct clk cpub_clk = {
	.name		= "cpub_clk",		
	.pmc_mask	= GBL_CLK_CPU_B,
	.type		= CLK_TYPE_CLK_CTL1,
};

static struct clk cpub_phl_clk = {
	.name		= "cpub_phl_clk",
	.parent 		= &g0_global_clk_half,
	.pmc_mask	= GBL_CLK_CPU_B_PHL,
	.type		= CLK_TYPE_CLK_CTL1,
};
...
MMP_ERR MMPF_PLL_GetPLLFreq(MMPF_GROUP_SRC pllNo, MMP_ULONG *ulPllFreq)
{
    ...
    switch(pllNo)
    {
        case MMPF_PLL_ID_0:  // PLL0
            ...
            LoopMulti = 1 << (pGBL->GBL_DPPL0_PARAM[0x01] & 0x01 ) ;              
            *ulPllFreq = (MMP_ULONG)(24000 * bN * LoopMulti) / (PostDiv * bM); 
            break;
        case MMPF_PLL_ID_1:  // PLL1
            bM = gbPostDiv[(pGBL->GBL_DPLL1_M & 0x07)];
            bN = pGBL->GBL_DPLL1_N;
            PostDiv = gbPostDiv[(pGBL->GBL_DPPL1_PARAM[0x00] & 0x07)];
            LoopMulti = 1 << (pGBL->GBL_DPPL1_PARAM[0x01] & 0x01 ) ;              
            *ulPllFreq = (MMP_ULONG)(24000 * bN * LoopMulti) / (PostDiv * bM); 
            break;
        case MMPF_PLL_ID_2:  // PLL2
            ...
            LoopMulti = 1 << (pGBL->GBL_DPPL2_PARAM[0x01] & 0x01 ) ;              
            *ulPllFreq = (MMP_ULONG)(24000 * bN * LoopMulti) / (PostDiv * bM); 
            break;
...
unsigned long preset_mcrv2_lpj ;
...
#define LPJ_528MHZ  1314816
  ...
  AITPS_GBL pGBL = AITC_BASE_GBL;
	...
	cpu_clk.rate_hz=g0_global_clk.rate_hz * ( (pGBL->GBL_CLK_DIV >> 1 ) + 1) ;
	preset_mcrv2_lpj = ( (cpu_clk.rate_hz/1000000) * LPJ_528MHZ ) / 528 ;
	
	printk(KERN_INFO"g0clk = %u,lpj = %u\n,",g0clk_khz,preset_mcrv2_lpj);
	/*
...
unsigned long ait_clock_lpj(void)
{
  return preset_mcrv2_lpj ;    
}
EXPORT_SYMBOL(ait_clock_lpj);
/*
