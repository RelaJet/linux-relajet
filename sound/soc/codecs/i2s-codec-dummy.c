...
#define WM8971_RATES (SNDRV_PCM_RATE_8000_192000)
...
static struct snd_soc_dai_driver i2s_codec_dummy_dai = {
	.name = "i2s-codec-dummy-hifi",
#if 0		
	.playback = {
...
static int wm8971_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, WM8971_LOUTM1, 0x0100, 0x0100);
	snd_soc_update_bits(codec, WM8971_ROUTM2, 0x0100, 0x0100);
...
static int __devinit i2s_codec_i2c_probe(struct platform_device *pdev){
	//struct wm8971_priv *wm8971;
	int ret;


	ret = snd_soc_register_codec(&pdev->dev,&soc_codec_dev_wm8971, &i2s_codec_dummy_dai, 1);

	return ret;
}

static __devexit int i2s_codec_i2c_remove(struct i2c_client *client)
{
...
static struct platform_driver i2s_codec_dummy_driver = {
	.driver = {
		.name = "i2s-codec-dummy",
		.owner = THIS_MODULE,
	},
	.probe =    i2s_codec_i2c_probe,
	.remove =   __devexit_p(i2s_codec_i2c_remove),

};



static struct platform_device *i2s_codec_dummy_dev;

int __init i2s_codec_dummy_modinit(void)
{
	int ret;

	i2s_codec_dummy_dev = platform_device_alloc("i2s-codec-dummy", -1);
	if (!i2s_codec_dummy_dev)
		return -ENOMEM;

	ret = platform_device_add(i2s_codec_dummy_dev);
	if (ret != 0) {
		platform_device_put(i2s_codec_dummy_dev);
		return ret;
	}

	ret = platform_driver_register(&i2s_codec_dummy_driver);
	if (ret != 0)
		platform_device_unregister(i2s_codec_dummy_dev);

	return ret;

	
}
module_init(i2s_codec_dummy_modinit);

static void __exit i2s_codec_dummy_exit(void)
{
	platform_device_unregister(i2s_codec_dummy_dev);
	platform_driver_unregister(&i2s_codec_dummy_driver);

}
module_exit(i2s_codec_dummy_exit);

MODULE_DESCRIPTION("AIT MCRV2 I2S Codec dummy driver");
