	...
	struct snd_pcm_substream *substream)
{
	...
	if (afe_sr & AFE_INT_FIFO_REACH_UNRD_TH && prtd->period_ptr){
		...
		unReadCntInPeriod = pAFE->AFE_FIFO_RD_TH/params->pdc_xfer_size;	//For 2 bye per sample
	...
	else if (afe_sr & AFE_INT_FIFO_REACH_UNWR_TH && prtd->period_ptr)
	{
		...
		//u32 unWriteCnt = 0x80; //(0x3F0- (u32)pAFE->AFE_FIFO_WR_TH)/2; // Fifo max size is 0x400 , and HW is 2CH only 
		u32 unWriteCnt = (u32)pAFE->AFE_FIFO_UNWR_CNT;
		if(prtd->channels==1)
...
#if (CHIP==VSN_V3)
	pAFE->AFE_FIFO_RD_TH = prtd->period_size/prtd->params->pdc_xfer_size;// 2*16000*2/1000;//0x80;
#endif	
	...
	int cmd)
{
	...
	switch (cmd) {
...
#if (CHIP==VSN_V3)
			pAFE->AFE_FIFO_CPU_INT_EN = AUD_INT_FIFO_REACH_UNRD_TH |AUD_INT_FIFO_FULL;
			pAFE->AFE_MUX_MODE_CTL = AFE_MUX_AUTO_MODE;
			pAFE->AFE_INT_CSR = 1;
#else if (CHIP==MCR_V2)
		if(dir == SNDRV_PCM_STREAM_CAPTURE)
		{
			...
			pAFE->AFE_FIFO_RST = 0;	
			pAFE->AFE_FIFO_CPU_INT_EN |= AUD_INT_FIFO_REACH_UNRD_TH |AFE_INT_FIFO_FULL;
			pAFE->AFE_MUX_MODE_CTL = AUD_MUX_AUTO;
			pAFE->AFE_CPU_INT_EN |= AUD_ADC_INT_EN;
		#else
		...
		else if(dir == SNDRV_PCM_STREAM_PLAYBACK)
		{
			...
			while(i--); // wait for one cycle of audio moudle clock
			pAFE->AFE_FIFO_RST = 0;	
			pAFE->AFE_FIFO_CPU_INT_EN |= AUD_INT_FIFO_REACH_UNWR_TH | AUD_INT_FIFO_EMPTY;
                	pAFE->AFE_L_CHNL_DATA = 0;
                	pAFE->AFE_R_CHNL_DATA = 0;
			pAFE->AFE_CPU_INT_EN |= AUD_DAC_INT_EN;	
			pAFE->AFE_MUX_MODE_CTL = AUD_MUX_AUTO;
		#else
		...
		{
			pAFE->AFE_FIFO_CPU_INT_EN |= AFE_INT_FIFO_REACH_UNWR_TH;
			pAFE->AFE_FIFO_CPU_INT_EN |= AUD_DAC_INT_EN;		
		}
		default:
			ret = -EINVAL;
	}
