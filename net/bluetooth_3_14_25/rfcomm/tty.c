...
#include <net/bluetooth_3_14_25/bluetooth.h>
#include <net/bluetooth_3_14_25/hci_core.h>
#include <net/bluetooth_3_14_25/rfcomm.h>
...
static int rfcomm_dev_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	while (1) {
		...
		tty_unlock(); //tty_unlock(tty); /*CONFIG_BT_BLE_3_14_25*/
		schedule();
		tty_lock(); //tty_lock(tty); /*CONFIG_BT_BLE_3_14_25*/
	}
...
static void rfcomm_dev_data_ready(struct rfcomm_dlc *dlc, struct sk_buff *skb)
{
	...
	tty_insert_flip_string(tty/*&dev->port*/, skb->data, skb->len);
	tty_flip_buffer_push(tty/*&dev->port*/);
...
{
	struct tty_struct *tty = dev->port.tty; /*3_5_0*/
	struct sk_buff *skb;
	int inserted = 0;
	...
	while ((skb = skb_dequeue(&dev->pending))) {
		inserted += tty_insert_flip_string(tty/*&dev->port*/, skb->data,
				skb->len);
		kfree_skb(skb);
	}
