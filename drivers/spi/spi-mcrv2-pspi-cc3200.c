...
//			SPI_SET_RXDMA_ADDR(as, as->buffer_dma);
			SPI_SET_RXDMA_ADDR(as, xfer->rx_dma);
	...
	SPI_SET_RXFIFO_THD(as,as->rxlen);


	if(1)	//fullduplex
	{
		SPI_SET_CFG(as,spi_cfg );
		SPI_SET_RXFIFO_THD(as,0);			
		SPI_SET_TXFIFO_THD(as,0);				
	}
	else
	{
