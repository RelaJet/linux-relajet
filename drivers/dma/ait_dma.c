/*
 * Driver for the AIT DMA Controller
 *
...
static int m2m_hw_setup(struct ep93xx_dma_chan *edmac)
{
	...
	u32 control = DMA_GET_INT(edmac);	// Enable DMA INT

	BUG_ON(edmac->chan_id>1);
	
	//pr_info("AIT DMA%d Setup\n", edmac->chan_id);
...
//	m2p_set_control(edmac, control);
	DMA_SET_INT(edmac,control);	// Enable DMA INT
	edmac->enabled = 1;
	return 0;
}
...
static void m2m_hw_shutdown(struct ep93xx_dma_chan *edmac)
{
	...
	u32 control = DMA_GET_CTL(edmac);
	pr_info("AIT DMA%d Shutdown\n", edmac->chan_id);
	
	if(edmac->chan_id)
	{
	       control &= ~DMA_M1_ENABLE;		
	}
	else
	       control &= ~DMA_M0_ENABLE;

//	DMA_SET_CTL(edmac,control);
	DMA_SET_INT(edmac,control);
	DMA_SET_SR(edmac,1<<edmac->chan_id);	
	edmac->enabled = 0;
...
static void m2m_fill_desc(struct ep93xx_dma_chan *edmac)
{
	...
	if(edmac->chan_id)
	{
		DMA_SET_DMA1_SRC_ADDR(edmac,desc->src_addr);
		DMA_SET_DMA1_DST_ADDR(edmac,desc->dst_addr);
		DMA_SET_DMA1_BYTE_COUNT(edmac,desc->size);
	}
	else
	{
		DMA_SET_DMA0_SRC_ADDR(edmac,desc->src_addr);
		DMA_SET_DMA0_DST_ADDR(edmac,desc->dst_addr);
		DMA_SET_DMA0_BYTE_COUNT(edmac,desc->size);
	}
...
static void m2m_hw_submit(struct ep93xx_dma_chan *edmac)
{
	...
	DMA_SET_CTL(edmac,control);		//Start DMA
}

static int m2m_hw_interrupt(struct ep93xx_dma_chan *edmac)
{
	//u32 control;
	u32 int_en ;//= DMA_GET_INT(edmac);
	u32 int_sr;// = DMA_GET_SR(edmac);

	if(edmac->enabled == 0)
		return INTERRUPT_UNKNOWN;
	
	int_en = DMA_GET_INT(edmac);
	int_sr = DMA_GET_SR(edmac);
	pr_info("int_en=0x%x\n",int_en);
	pr_info("int_sr=0x%x\n",int_sr);	

	if(!((int_en &int_sr)&1<<edmac->chan_id))//(DMA_INT_M0|DMA_INT_M1))
		return INTERRUPT_UNKNOWN;

	/* Clear the DONE bit */
	int_sr &= int_en;
	DMA_SET_SR(edmac,int_sr);
	
	/* Disable interrupts and the channel */
	int_en &=~int_sr;
	//DMA_SET_INT(edmac,int_en);
...
static int ep93xx_dma_alloc_chan_resources(struct dma_chan *chan)
{
	...
	ret = request_irq(edmac->irq, ep93xx_dma_interrupt, IRQF_SHARED, name, edmac);
	if (ret)
...
static int __init ep93xx_dma_probe(struct platform_device *pdev)
{
	...
	edma->m2m = 1;// platform_get_device_id(pdev)->driver_data;
	...
	for (i = 0; i < pdata->num_channels; i++) {
		...
		edmac->clk = clk_get( NULL,"dma_clk");
		if (IS_ERR(edmac->clk)) {
		...
		tasklet_init(&edmac->tasklet, ep93xx_dma_tasklet, (unsigned long)edmac);
...
static struct platform_device_id ait_dma_driver_ids[] = {
	{ "ait-dma-m2m", 0 },
	{ "ait-dma-m2m", 1 },
	{ },
};
