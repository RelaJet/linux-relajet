...
/* Disable support for DDR QUAD and OCTAL modes for now.
   This support requires update of the common spi-nor code. */
#define SUPPORT_DDR_QUAD_OCTAL 0
...
static struct fsl_flexspi_devtype_data imxrt1050_data = {
	.devtype = FSL_FLEXSPI_IMXRT1050,
	.rxfifo = 128,
	.txfifo = 128,
	.ahb_buf_size = 1024,
	.driver_data = 0,
	...
	if (nor->flash_read == SPI_NOR_QUAD) {
		if (op == SPINOR_OP_READ_1_1_4 || op == SPINOR_OP_READ4_1_1_4) {
			...
			writel(LUT0(DUMMY, PAD1, dm) |
			       ...
			       base + FLEXSPI_LUT(lut_base + 1));
		} else if (op == SPINOR_OP_READ_1_4_4) {
			/* read mode : 1-4-4 */
			writel(LUT0(CMD, PAD1, op) | LUT1(ADDR, PAD4, addrlen),
				base + FLEXSPI_LUT(lut_base));
...
static void fsl_flexspi_write(struct spi_nor *nor, loff_t to,
		size_t len, size_t *retlen, const u_char *buf)
{
	struct fsl_flexspi *flex = nor->priv;
	size_t ret;

	ret = fsl_flexspi_nor_write(flex, nor, nor->program_opcode, to,
				(u32 *)buf, len);
...
static int fsl_flexspi_read(struct spi_nor *nor, loff_t from,
...
static const struct of_device_id fsl_flexspi_dt_ids[] = {
	...
	{ .compatible = "fsl,imxrt1050-flexspi", .data = (void *)&imxrt1050_data, },
	{ /* sentinel */ }
};
