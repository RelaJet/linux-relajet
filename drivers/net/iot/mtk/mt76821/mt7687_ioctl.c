...
static int mt7687_ioctl_g_info(struct net_device *ndev,  iot_ioctl_req_t __user *pInfoReq)
{
	...
	ret = iot_read_data(&host->mt_spi, SPIS_CMD_GET_INFO, &devinfo, (u32)sizeof(mt7687_dev_info_t));
	...
	pr_info("ssid: %s\n",	(int)devinfo.wifisetting.ssid);
	pr_info("psk: %s\n",	(int)devinfo.wifisetting.psk);
...
static int mt7687_ioctl_s_powermode(struct net_device *ndev,  iot_ioctl_req_t __user *pReq)
{
	...
	switch(req.subcmd)
	{
		...
		case IOT_SET_IOT_STANDBY:
		{
			...
			down (&host->spi_lock);	
			ret = iot_set_power(&host->mt_spi, 0);
			up (&host->spi_lock);
 ...
 */
//static 
u32 mt7687_get_link(struct net_device *ndev)
{
	u32 link = 0;
	int ret;
	PMT_PRIVATE host = (PMT_PRIVATE)netdev_priv(ndev);
	...
	down (&host->spi_lock);
	ret = iot_read_data(&host->mt_spi, SPIS_CMD_GET_LINK, &link, 4);
	up (&host->spi_lock);
