...
int iot_get_stat(int fd, const char *if_name, iot_ioctl_statistics_t* pStat)
{
    struct ifreq ifr;
    struct ethtool_value edata;
    edata.cmd = 0;
    edata.data =(u32) pStat;
    memset(&ifr, 0, sizeof(ifr));
    strncpy(ifr.ifr_name, if_name, sizeof(ifr.ifr_name) - 1);
    ifr.ifr_data = (char *) &edata;
    if(ioctl( fd, SIOCG_STATISTICS, &ifr  ) == -1)
    {
	printf("ioctrl: get statistics fail.\r\n");
	return -1;
    }
    return 0;
}
...
void help(char *progname)
{
  ...
  fprintf(stderr, "Usage: %s\n" \
                  ...
                  " [-up].......: Interface up.\n" \
                  " [-a | --auth ]......: Auth mode. 0: open 1:wep 2:wpa/wpa2\n" \
...
int main(int argc, char* argv[])
{
	...
	if(get_stat)
	{
	  iot_ioctl_statistics_t stat;
	  setifflags(skfd, pIFname, IFF_UP);
	  if(iot_get_stat(skfd,pIFname, &stat)==0)
	  {
	    printf("CC3200 driver statistics:\r\n");
	    printf("tcp tx count: %d\r\n",stat.drv_buf_tcptx_count);
	    printf("udp tx count: %d\r\n",stat.drv_buf_udptx_count);
	  }
	  else
	  {
	      printf("get statistic error\r\n");
	  }
	  goto _exit;
	}
	if(bIsGetInfo)
...
//				iot_set_ifaddr(skfd, pIFname, info.ipaddr);
	...
	if(senddata)
	{
	    ...
	    iov.iov_base = buf;
	    iov.iov_len = 512;
	    if(ret=ioctl( skfd, SIOCS_DATA_SEND, &iov ) == -1)
	    {
		return -1;
	    }    
	}
