...
static void ax88796c_dump_regs (PAX_PRIVATE ax_local)
{
	u8 i, j;
printk(KERN_INFO "%s.\n",__func__);
	printk (KERN_INFO"       Page0   Page1   Page2   Page3   "
				"Page4   Page5   Page6   Page7\n");

	for (i = 0; i < 0x20; i += 2) {

		printk (KERN_INFO"0x%02x   ", i);
		for (j = 0; j < 8; j++) {
...
#if 0//def CONFIG_ARCH_VSNV3AIT845X
	unsigned long start;


	ax88796c_mdio_write(ax_local->ndev, ax_local->mii.phy_id, MII_BMCR, BMCR_RESET);

	start = jiffies;

	while (!(ax88796c_mdio_read(ax_local->ndev,ax_local->mii.phy_id,MII_BMCR) & BMCR_RESET))
	{
		if (time_after(jiffies, start +  HZ )) {
			netdev_err(ax_local->ndev, "PHY Reset would not complete\n");
			return -1;
		}
	}
...
static int ax88796c_reset (PAX_PRIVATE ax_local)
{
	...
	u16 temp;
#if 0
	MMPF_PIO_SetData(0x73, 0,0);
	mdelay(500);
	MMPF_PIO_SetData(0x73, 1,0);
	mdelay(500);
...
printk(KERN_INFO "P0_PSR = 0x%02x.\n",AX_READ (&ax_local->ax_spi, P0_PSR) );
...
printk(KERN_INFO "P4_SPICR = 0x%02x.\n",temp );	
...
printk(KERN_INFO "ax_local->ax_spi.comp = 0x%02x.\n",ax_local->ax_spi.comp );	
	return 0;
}
...
//int i=0,j;	
	...
	entry = (struct skb_data *) tx_skb->cb;
	//TSNR[0] = AX_READ (&ax_local->ax_spi, P0_TSNR);
	AX_WRITE (&ax_local->ax_spi,
			(TSNR_TXB_START | TSNR_PKT_CNT(1)), P0_TSNR);
	//TSNR[1] = AX_READ (&ax_local->ax_spi, P0_TSNR);
//TFBFCR[0] = AX_READ (&ax_local->ax_spi, P0_TFBFCR);

	axspi_write_txq (&ax_local->ax_spi, tx_skb->data, tx_skb->len);
//TFBFCR[1] = AX_READ (&ax_local->ax_spi, P0_TFBFCR);
	
//	TSNR[2] = AX_READ (&ax_local->ax_spi, P0_TSNR);
#if 0	
	for(j=0;j<100;++j)
	{
	...
	if(i)
		printk(KERN_INFO "TXNR_TXB_IDLE = %d times\n",i);
#endif
	if (((AX_READ (&ax_local->ax_spi, P0_TSNR) & TXNR_TXB_IDLE) == 0) ||
	    ((ISR_TXERR & AX_READ (&ax_local->ax_spi, P0_ISR)) != 0)) {
//		static int i;
		unsigned long start;
//printk(KERN_INFO"TFBFCR %02x %02x \n",TFBFCR[0],TFBFCR[1]);

		
//printk(KERN_INFO"TSNR %02x %02x %02x \n",TSNR[0],TSNR[1],TSNR[2]);

printk(KERN_INFO"P0_ISR = %x\n",AX_READ (&ax_local->ax_spi, P0_ISR));				
printk(KERN_INFO"3 P0_TSNR = %x    tx_skb->len = %d\n",AX_READ (&ax_local->ax_spi, P0_TSNR) , tx_skb->len);	
		start = jiffies;
...
#else
static irqreturn_t 
ax88796c_interrupt (int irq, void *dev_instance, struct pt_regs * regs)
#endif
{
...
//printk(KERN_INFO "%s.\n",__func__);
	...
	if (!(isr & (ISR_TXERR|ISR_TXPAGES|ISR_LINK|ISR_RXPKT)))
		return IRQ_NONE;
#endif
...
//printk(KERN_INFO "EVENT_WATCHDOG\n");
...
//	.ndo_set_multicast_list = ax88796c_set_multicast,
	...
	.ndo_set_rx_mode = ax88796c_set_multicast,
#else
	.ndo_set_multicast_list = ax88796c_set_multicast,
#endif	
...
printk(KERN_INFO "%s.\n",__func__);
	...
	ret = gpio_request_one(reset_pin,GPIOF_DIR_OUT|GPIOF_INIT_LOW,"GPIO_ETHERNET_RESET");
	if(ret<0)
		return ret;

	printk(KERN_INFO "Request reset_pin %d success.\n",reset_pin);
	
//	ait_set_gpio_output(0x73,0);
//	ait_set_gpio_value(0x73, 0);
	msleep(500);
	//ait_set_gpio_value(0x73, 1);
...

	{
		...
		temp = AX_READ (&ax_local->ax_spi, P0_BOR);
		printk(KERN_INFO "P0_BOR = 0x%02x.\n",temp);
		
		if (temp == 0x1234) {
			AX_WRITE (&ax_local->ax_spi, 0	, P0_BOR);		
	...
	//Vin: init_MUTEX (&ax_local->spi_lock);
...
ax88796c_suspend (struct spi_device *spi, pm_message_t mesg)
{
	...
	u8 power;
printk(KERN_INFO "ax88796c_suspend.\n");
...
ax88796c_resume (struct spi_device *spi)
{
	...
	struct net_device *ndev = ax_local->ndev;
	u16 pme;
printk(KERN_INFO "ax88796c_resume.\n");
