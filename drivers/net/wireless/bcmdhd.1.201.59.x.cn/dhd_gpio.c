...
)
{
	...
	if (on) {
		if (gpio_wl_reg_on >= 0) {
#if defined (CUSTOMER_HW_MSTAR) && defined (DHD_LOAD_CHIPALIVE)
...
#ifdef CUSTOMER_HW_MSTAR
EXPORT_SYMBOL(bcm_wlan_set_power);
#endif
...
int dhd_wlan_init_gpio(void)
{
	...
	*/
	gpio_wl_reg_on = AIT_GPIO_WIFI_RESET;
#ifdef CUSTOMER_OOB
	gpio_wl_host_wake = AIT_GPIO_WIFI_OOB_IRQ;
#endif

	printf("%s: GPIO(WL_REG_ON) = %d\n", __FUNCTION__, gpio_wl_reg_on);
	if (gpio_wl_reg_on >= 0) {
#if defined  (CUSTOMER_HW_MSTAR)  && defined (DHD_LOAD_CHIPALIVE)
		err = gpio_request_one(gpio_wl_reg_on, GPIOF_DIR_OUT|GPIOF_INIT_LOW, "WL_REG_ON");
#else
		err = gpio_request_one(gpio_wl_reg_on, GPIOF_DIR_OUT|GPIOF_INIT_HIGH, "WL_REG_ON");
