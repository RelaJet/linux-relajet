...
dhd_conf_get_mac(dhd_pub_t *dhd, bcmsdh_info_t *sdh, uint8 *mac)
{
	...
	do {
		...
		if (tpl_link == 0xff)
			break;
		if (config_msg_level & CONFIG_TRACE_LEVEL) {
			DHD_PRINT(("%s: tpl_code=0x%02x, tpl_link=0x%02x, tag=0x%02x\n",
				__FUNCTION__, tpl_code, tpl_link, *ptr));
			DHD_PRINT(("%s: value:", __FUNCTION__));
			for (i=0; i<tpl_link-1; i++) {
				DHD_PRINT(("%02x ", ptr[i+1]));
				if ((i+1) % 16 == 0)
					DHD_PRINT(("\n"));
			}
			DHD_PRINT(("\n"));
...
dhd_conf_set_fw_name_by_mac(dhd_pub_t *dhd, bcmsdh_info_t *sdh, char *fw_path)
{
	...
	for (i=0; i<fw_num; i++) {
		...
		if (fw_type != fw_type_new) {
			DHD_PRINT(("%s: fw_typ=%d != fw_type_new=%d\n", __FUNCTION__, fw_type, fw_type_new));
			continue;
		}
		for (j=0; j<mac_num; j++) {
			if (oui == mac_range[j].oui) {
				if (nic >= mac_range[j].nic_start && nic <= mac_range[j].nic_end) {
					...
					DHD_PRINT(("%s: matched oui=0x%06X, nic=0x%06X\n",
...
dhd_conf_set_country(dhd_pub_t *dhd)
{
	...
	DHD_PRINT(("%s: set country %s, revision %d\n", __FUNCTION__,
...
dhd_conf_update_wme(dhd_pub_t *dhd, edcf_acparam_t *acparam_cur, int aci)
{
	...
	DHD_PRINT(("%s: mod aci %d aifsn %d ecwmin %d ecwmax %d txop 0x%x\n",
		__FUNCTION__,
...
dhd_conf_get_disable_proptx(dhd_pub_t *dhd)
{
	...
	DHD_PRINT(("%s: fw_proptx=%d, disable_proptx=%d\n", __FUNCTION__, fw_proptx, disable_proptx));
...
dhd_conf_read_fw_by_mac(dhd_pub_t *dhd, char *bufp, uint len)
{
	...
	if (len_val) {
		...
		for (i=0; i<conf->fw_by_mac.count; i++) {
			...
			DHD_PRINT(("%s: name=%s, mac_count=%d\n", __FUNCTION__,
			...
			for (j=0; j<mac_list[i].count; j++) {
				...
				DHD_PRINT(("%s: oui=0x%06X, nic_start=0x%06X, nic_end=0x%06X\n",
...
dhd_conf_read_nv_by_chip(dhd_pub_t *dhd, char *bufp, uint len)
{
	...
	if (len_val) {
		...
		for (i=0; i<conf->nv_by_chip.count; i++) {
			...
			DHD_PRINT(("%s: chip=0x%x, chiprev=%d, name=%s\n", __FUNCTION__,
...
dhd_conf_read_roam_params(dhd_pub_t *dhd, char *bufp, uint len)
{
	...
	if (len_val) {
		...
		DHD_PRINT(("%s: roam_off_suspend = %d\n", __FUNCTION__,
	...
	if (!conf->roam_off || !conf->roam_off_suspend) {
		...
		DHD_PRINT(("%s: roam_scan_period = %d\n", __FUNCTION__,
...
dhd_conf_read_config(dhd_pub_t *dhd, char *conf_path)
{
	...
	if (len > 0 && len < MAXSZ_CONFIG) {
		...
		if (len_val) {
			...
			DHD_PRINT(("%s: channels = ", __FUNCTION__));
			for (i=0; i<conf->channels.count; i++)
				DHD_PRINT(("%d ", conf->channels.channel[i]));
			DHD_PRINT(("\n"));
		}
		...
		if (len_val) {
			...
			DHD_PRINT(("%s: keep_alive_period = %d\n", __FUNCTION__,
		...
		if (len_val) {
			...
			DHD_PRINT(("%s: pkt_filter_del id = ", __FUNCTION__));
			for (i=0; i<conf->pkt_filter_del.count; i++)
				DHD_PRINT(("%d ", conf->pkt_filter_del.id[i]));
			DHD_PRINT(("\n"));
		}
		...
		if (len_val) {
			...
			while (pch != NULL && i<DHD_CONF_FILTER_MAX) {
				...
				DHD_PRINT(("%s: magic_pkt_filter_add[%d][] = %s\n",
		...
		if (len_val) {
			...
			DHD_PRINT(("%s: num_different_channels = %d\n",
...
dhd_conf_set_txglom_params(dhd_pub_t *dhd, bool enable)
{
	...
	DHD_PRINT(("%s: swtxglom=%d, txglom_ext=%d, txglom_bucket_size=%d\n", __FUNCTION__,
		conf->swtxglom, conf->txglom_ext, conf->txglom_bucket_size));
	DHD_PRINT(("%s: txglomsize=%d, deferred_tx_len=%d, bus_txglom=%d\n", __FUNCTION__,
		conf->txglomsize, conf->deferred_tx_len, conf->bus_txglom));
	DHD_PRINT(("%s: tx_in_rx=%d, txinrx_thres=%d, dhd_txminmax=%d\n", __FUNCTION__,
		conf->tx_in_rx, conf->txinrx_thres, conf->dhd_txminmax));
	DHD_PRINT(("%s: tx_max_offset=%d, txctl_tmo_fix=%d\n", __FUNCTION__,
...
dhd_conf_preinit(dhd_pub_t *dhd)
{
	...
	if (conf->chip == BCM43430_CHIP_ID || conf->chip == BCM4345_CHIP_ID) {
		...
		// terence 20170217: fix me, this is temporary value for dhd_chip_alive
		conf->bus_rxglom = 0;
		conf->disable_proptx = 1;
#endif
	}
