...
#define CUSTOM_OOB_GPIO_NUM (AIT_GPIO_WIFI_OOB_IRQ)
...
#if 1
	{
		struct ether_addr ea_address = {{0x00, 0x11, 0x22, 0x33, 0x44, 0xFF}};
		u8 octet[6] = {0};
		int res = 0;
		//WL_ERROR(("%s: MAC_address %s\n", __FUNCTION__, MAC_address));

		res = sscanf(MAC_address, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &octet[0],
			&octet[1], &octet[2], &octet[3], &octet[4], &octet[5]);
		WL_ERROR(("%s: %x:%x:%x:%x:%x:%x, res = %d\n",__FUNCTION__, octet[0],octet[1],octet[2],octet[3],octet[4],octet[5],res));
		if (res < 0) {
			//struct ether_addr ea_address = {{0x00, 0x11, 0x22, 0x33, 0x44, 0xFF}};
			WL_ERROR(("%s: format response = %d\n", __FUNCTION__, res));
		} else {
			memcpy(ea_address.octet, octet, sizeof(octet));
		}
		bcopy((char *)&ea_address, buf, sizeof(struct ether_addr));
	}
#endif
