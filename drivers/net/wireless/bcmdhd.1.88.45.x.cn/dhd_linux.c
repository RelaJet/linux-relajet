...
char MAC_address[MOD_PARAM_ADDRESSLEN];
module_param_string(MAC_address, MAC_address, MOD_PARAM_PATHLEN, 0);
#endif
...
char iface_name[IFNAMSIZ] = "wlan";//{'\0'};
...
dhd_dpc_thread(void *data)
{
	 ...
	 * so get rid of all our resources
	 */
	if (dhd_dpc_prio > 0) /* By Siseong Ahn */
	{
		printk( "%s: FIFO ==> NORMAL\n", __func__ );
		struct sched_param param;
...
uint gpio_reset_pin = AIT_GPIO_WIFI_RESET;
...
MODULE_PARM_DESC(gpio_reset_pin, "WIFI Reset pin number from AIT84xx");
...
#ifndef CONFIG_AIT_FAST_BOOT
	int ret = gpio_request_one(gpio_reset_pin,GPIOF_DIR_OUT|GPIOF_INIT_HIGH,"GPIO_WF_RESET");
	if(ret<0)
		goto fail_1;
...
#endif
		if (down_timeout(&dhd_chipup_sem,	// Vin : check boot time
...
#define MAX_WAIT_FOR_8021X_TX	200
