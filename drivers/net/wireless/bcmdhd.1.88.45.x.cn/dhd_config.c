...
dhd_conf_set_stbc(dhd_pub_t *dhd)
{
	...
	if (dhd_bus_chip_id(dhd) == BCM4324_CHIP_ID && dhd->conf->stbc >= 0) {
...
dhd_conf_set_phyoclscdenable(dhd_pub_t *dhd)
{
	...
	if (dhd_bus_chip_id(dhd) == BCM4324_CHIP_ID && dhd->conf->phy_oclscdenable >= 0) {
...
dhd_conf_read_config(dhd_pub_t *dhd)
{
	...
	if (len > 0 && len < MAXSZ_CONFIG) {
		...
		if (len_val) {
			...
			dhd->conf->srl = 15; // By Siseong Ahn
		...
		if (len_val) {
			...
			dhd->conf->lrl = 15; // By Siseong Ahn
...
#endif
	dhd->conf->srl = 15; //-1;  // By Siseong Ahn
	dhd->conf->lrl = 15; //-1; // By Siseong Ahn
	dhd->conf->bcn_timeout = 15;
