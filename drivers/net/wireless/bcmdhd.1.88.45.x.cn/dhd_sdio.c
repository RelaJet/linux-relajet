...
#define BCMEMBEDIMAGE
#define DUMP_RAM_EN       (0)
#define PHY_FW_MEM_ADDR    ((0x01000000) + ((62 * 1024 - 260 ) * 1024 ))
#define PHY_NVRAM_MEM_ADDR ((0x01000000) + ((62 * 1024 - 4   ) * 1024 ))
#endif
...
#define QLEN		3072 // 256 ==> 1024 By Siseong Ahn	/* bulk rx and tx queue lengths */
#define FCHI		(QLEN - 10)
#define FCLOW		(FCHI / 2)
#define PRIOMASK	7
...
#ifndef DHD_RXBOUND
#ifdef CONFIG_HIS
#define DHD_RXBOUND	8	/* Default for max rx frames in one scheduling */
#else
	...
	if (bus->glom_enable) {
		...
		if (queue_only) {
			...
			if(bus->glom_cnt == 0) {
				...
				len = bus->txglom_bucket_size - TXGLOM_RECV_OFFSET;
			} else {
				// add pad to bucket size
				len = bus->txglom_bucket_size;
			}
#else
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	if (bus->sih->chip == BCM43362_CHIP_ID || bus->sih->chip == BCM4330_CHIP_ID)
		bus->txglom_bucket_size = 1680;
	else
		bus->txglom_bucket_size = 1684;
	printf("%s: txglom_bucket_size=%d\n", __FUNCTION__, bus->txglom_bucket_size);
#endif
#endif
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	if ((bus->sih->chip == BCM43362_CHIP_ID) || (bus->sih->chip == BCM4330_CHIP_ID)) {
		DHD_INFO(("%s: Disable prop_txstatus\n", __FUNCTION__));
		disable_proptx = 1;
	}
#endif
...
#else
  printk("Load (wifi firmware/nvram) from : 0x%08x,0x%08x\n",PHY_FW_MEM_ADDR,PHY_NVRAM_MEM_ADDR) ;
#endif
...
void dump_ram_info(unsigned char *data)
{
#if DUMP_RAM_EN  
  int i ;
  pr_info("RAM addr : 0x%08x\n",data);
  for(i=0;i<16;i++) {
    pr_info("%02x ",data[i]);
  }
  pr_info("\n");
...
dhdsdio_download_code_array(struct dhd_bus *bus)
{
	...
	//unsigned char *dlarray= (char *)0xf3800000;	
	
	unsigned char *dlarray= (unsigned char *)(0xf0000000 | PHY_FW_MEM_ADDR );	
	
	dump_ram_info(dlarray) ;
	
	DHD_INFO(("%s: download embedded firmware...\n", __FUNCTION__));
	/* Download image */
	//while ((offset + MEMBLOCK*120) < 246464 /*sizeof(dlarray)*/) 
	{
		/* check if CR4 */
		if (si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
		...
		bcmerror = dhdsdio_membytes(bus, TRUE, offset,
			(uint8 *) (dlarray + offset), 246464);
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
			        __FUNCTION__, bcmerror, 246464, offset));
			goto err;
		}
	...
	while ((offset + MEMBLOCK*120) < 246464 /*sizeof(dlarray)*/) {
		...
		bcmerror = dhdsdio_membytes(bus, TRUE, offset,
			(uint8 *) (dlarray + offset), MEMBLOCK*120);
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
			        __FUNCTION__, bcmerror, MEMBLOCK*120, offset));
			goto err;
		}
	...
	if (offset < 246464 /*sizeof(dlarray)*/) {
		bcmerror = dhdsdio_membytes(bus, TRUE, offset,
			(uint8 *) (dlarray + offset), 246464 /*sizeof(dlarray)*/ - offset);
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
			        __FUNCTION__, bcmerror, 246464 /*sizeof(dlarray)*/ - offset, offset));
			goto err;
		}
	}
#endif
	...
	/* Upload and compare the downloaded code */
	{
		...
		while ((offset + MEMBLOCK*120) < 246464 /*sizeof(dlarray)*/) {
			bcmerror = dhdsdio_membytes(bus, FALSE, offset, ularray + offset, MEMBLOCK*120);
			if (bcmerror) {
		...
		if (offset < 246464 /*sizeof(dlarray)*/) {
			bcmerror = dhdsdio_membytes(bus, FALSE, offset,
				ularray + offset, 246464 /*sizeof(dlarray)*/ - offset);
			if (bcmerror) {
				DHD_ERROR(("%s: error %d on reading %d membytes at 0x%08x\n",
					__FUNCTION__, bcmerror, 246464 /*sizeof(dlarray)*/ - offset, offset));
				goto err;
		...
		if (memcmp(dlarray, ularray,  246464 /*sizeof(dlarray)*/)) {
...
#ifdef BCMEMBEDIMAGE
static int
dhdsdio_download_nvram_array(struct dhd_bus *bus)
{
	int bcmerror = -1;
	uint len;
	void * image = NULL;
	...
	//memblock = (char*) 0xf3840000;//MALLOC(bus->dhd->osh, MAX_NVRAMBUF_SIZE);
	memblock = (char*)(0xf0000000 | PHY_NVRAM_MEM_ADDR );
  dump_ram_info((unsigned char *)memblock);
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	bool embed = TRUE;	/* download embedded firmware */
	...
	if (dhdsdio_download_nvram_array(bus)) {
