...
dhd_conf_get_mac(dhd_pub_t *dhd, bcmsdh_info_t *sdh, uint8 *mac)
{
	int i, err = -1;
	uint8 *ptr = 0;
	unsigned char tpl_code, tpl_link=NULL;
...
dhd_conf_set_fw_name_by_chip(dhd_pub_t *dhd, char *fw_path)
{
	...
	if (fw_path[0] == '\0') {
		...
		bcm_strncpy_s(fw_path, MOD_PARAM_PATHLEN-1, CONFIG_BCMDHD_AP6330_FW_PATH, MOD_PARAM_PATHLEN-1);
		if (fw_path[0] == '\0')
#endif
		{
...
dhd_conf_add_pkt_filter(dhd_pub_t *dhd)
{
	...
	/*
	All pkt: pkt_filter_add=99 0 0 0 0xFFFFFFFFFFFF 0x000000000000
