...
//===============add 2014===================
	if(auto_ch){
		...
		if((wl_cfg80211_get_chanspecs_2g(ndev, (void *)buf, CHANSPEC_BUF_SIZE))<0){
			if((wl_cfg80211_get_chanspecs_5g(ndev, (void *)buf, CHANSPEC_BUF_SIZE))){
					goto error;
				}
			}
		wl_cfg80211_get_best_channel(ndev,(void *)buf ,CHANSPEC_BUF_SIZE,&channel);
error:
