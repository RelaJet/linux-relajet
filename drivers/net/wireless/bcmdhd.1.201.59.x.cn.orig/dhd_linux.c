...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	/* Reject if down */
	if (dhd->pub.busstate == DHD_BUS_DOWN || dhd->pub.hang_was_sent || dhd->pub.early_suspended) {
		DHD_ERROR(("%s: xmit rejected pub.up=%d busstate=%d \n",
			__FUNCTION__, dhd->pub.up, dhd->pub.busstate));
		netif_stop_queue(net);
		/* Send Event when bus down detected during data session */
		if (dhd->pub.up && !dhd->pub.early_suspended) {
...
fail_retry:
			...
			if (ret != 0) {
				DHD_ERROR(("%s : wl_android_wifi_on failed (%d) start %dst retry\n",
					__FUNCTION__, ret, retry++));
				if (dhd_chip_alive) {
					printf("Turn on wifi error. Turn off dhd_chip_alive.\n");
					dhd_chip_alive = 0;
				}
...
dhd_alive_preinit(dhd_pub_t *dhd)
{
	...
	if (op_mode == 0) {
		...
		DHD_PRINT(("%s: op_mode must be assigned if chip is alive\n", __FUNCTION__));
	}
...
dhd_alive_postinit(dhd_pub_t *dhd)
{
...
#ifdef ARP_OFFLOAD_SUPPORT
	dhd_conf_get_iovar(dhd, WLC_GET_VAR, "arpoe", (char *)&arpoe, sizeof(arpoe), 0);
	if (arpoe) {
		ret = dhd_conf_get_iovar(dhd, WLC_GET_VAR, "arp_version", (char *)&version, sizeof(version), 0);
		if (ret) {
			dhd->arp_version = 1;
		} else {
	...
	/* Get the default device MAC address directly from firmware */
	dhd_conf_get_iovar(dhd, WLC_GET_VAR, "cur_etheraddr", (char *)dhd->mac.octet, ETHER_ADDR_LEN, 0);

	/* get a capabilities from firmware */
	memset(dhd->fw_capabilities, 0, sizeof(dhd->fw_capabilities));
	dhd_conf_get_iovar(dhd, WLC_GET_VAR, "cap", (char *)dhd->fw_capabilities, sizeof(dhd->fw_capabilities), 0);

	dhd_conf_get_country(dhd, &dhd->dhd_cspec);

#ifdef WLTDLS
	if (FW_SUPPORTED(dhd, tdls)) {
		ret= dhd_conf_get_iovar(dhd, WLC_GET_VAR, "tdls_enable", (char *)&tdls, sizeof(tdls), 0);
		if (ret)
			dhd->tdls_enable = FALSE;
		else
	...
	memset(buf, 0, sizeof(buf));
	ptr = buf;
	dhd_conf_get_iovar(dhd, WLC_GET_VAR, "ver", (char *)buf, sizeof(buf), 0);
	if (!ret) {
		bcmstrtok(&ptr, "\n", 0);
		/* Print fw version info */
		dhd_set_version_info(dhd, buf);
	...
	// trigger WLC_E_FIFO_CREDIT_MAP and WLC_E_BCMC_CREDIT_SUPPORT to sync for dhd driver
	if (!ret)
		dhd_conf_get_iovar(dhd, WLC_GET_VAR, "proptx_credit_map", (char *)buf, sizeof(buf), 0);
#endif
