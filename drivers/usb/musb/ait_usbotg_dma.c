...
//todo	musb_dma_completion(musb, hw_ep->epnum,
	//			ait_usbdma_channel->is_tx);
	spin_unlock_irqrestore(&musb->lock, flags);
}
...
void ait_usbdma_callback(void *private_data)
{
	...
	//schedule_work(&ait_usbdma_channel->channel_work);
}
...
//if(len>1564)
	...
	{	
#if 0
		ait_usbdma_channel->cur_len = min(len,512*4);//
...
//		ch_num -= 8;
	...
	if (!(/*hw_ep->epnum==4||*/hw_ep->epnum==5))// && hw_ep->epnum!=3 && hw_ep->epnum!=1)
		return NULL;
		...
		u16 maxpacket, void *buf, u32 length)
{
	if ((maxpacket & 0x3)		||
		...
		//(length & 0x3))
	{
		pr_info("Not compatible!maxpacket = %d   buf = %d  length=%d\n",maxpacket,buf, length);
		return false;
	}
	else
		return true;
...
pr_info("ait_usbdma_controller_start  \n");
