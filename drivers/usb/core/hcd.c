...
nongeneric:
		...
		status = hcd->driver->hub_control (hcd,		//musb_hub_control
...
int usb_hcd_submit_urb (struct urb *urb, gfp_t mem_flags)
{
	...
	if (is_root_hub(urb->dev)) {
		...
		if (likely(status == 0)) {
			status = hcd->driver->urb_enqueue(hcd, urb, mem_flags);	//musb_urb_enqueue
			if (unlikely(status))
		...
		unsigned int irqnum, unsigned long irqflags)
{
	...
	hcd->state = HC_STATE_RUNNING;
	retval = hcd->driver->start(hcd);	//musb_h_start
	if (retval < 0) {
