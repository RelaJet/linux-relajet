 ...
 */
#if ENABLE_PLAYBACK 
#define PLAYBACK_EP_MAX_PACKET_SIZE	16 //32
...
//#define CAPTURE_EP_MAX_PACKET_SIZE  16 //for 8k
//#define CAPTURE_EP_MAX_PACKET_SIZE 96    //for 48k
#define CAPTURE_EP_MAX_PACKET_SIZE 88    //for 44.1k 
...
static int req_capture_count = 256;
...
//static int audio_capture_buf_size = 48000;
static int audio_capture_buf_size = 44100;
...
#define F_AUDIO_INTERFACE_MICROPHONE	1

#if ENABLE_PLAYBACK 
#define F_AUDIO_INTERFACE_SPEAKER	2
#define F_AUDIO_NUM_INTERFACES		2
#else
#define F_AUDIO_NUM_INTERFACES		1
#endif

//check: Interface Association Descriptor
...
//check: Interface Descriptor uac
...
struct uac_input_terminal_descriptor speaker_input_terminal_desc = {
	...
	.bTerminalID		= SPEAKER_INPUT_TERMINAL_ID, //3
...
struct uac1_output_terminal_descriptor speaker_output_terminal_desc = {
	...
	.bTerminalID		= SPEAKER_OUTPUT_TERMINAL_ID, //4
...
//check: Interface Intput Terminal Descriptor
static struct uac_input_terminal_descriptor microphone_input_terminal_desc = {
	.bLength		= UAC_DT_INPUT_TERMINAL_SIZE,
	.bDescriptorType	= USB_DT_CS_INTERFACE,
	.bDescriptorSubtype	= UAC_INPUT_TERMINAL,
	.bTerminalID		= MICROPHONE_INPUT_TERMINAL_ID, //1
...
//check: Interface Outtput Terminal Descriptor
static struct
uac1_output_terminal_descriptor microphone_output_terminal_desc = {
	.bLength		= UAC_DT_OUTPUT_TERMINAL_SIZE,
	.bDescriptorType	= USB_DT_CS_INTERFACE,
	.bDescriptorSubtype	= UAC_OUTPUT_TERMINAL,
	.bTerminalID		= MICROPHONE_OUTPUT_TERMINAL_ID, //2
	.wTerminalType		= UAC_TERMINAL_STREAMING,
...
static struct uac_feature_unit_descriptor_0 feature_unit_desc = {
	...
	.bUnitID	= FEATURE_UNIT_ID, //2
	.bSourceID	= MICROPHONE_INPUT_TERMINAL_ID, //1
	.bControlSize	= 2,
...
f_audio_playback_ep_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (audio_playback_buf_size - copy_buf->actual < req->actual) {
		printk("audio_playback_buf_size %d - copy_buf->actual %d, req->actual %d\n",
...
static void f_audio_capture_work(struct work_struct *data)
{
	...
	if (res)
		pr_err("copying failed");
	//printk("uac: Queue capture packet: size %d\n", audio_capture_buf_size);
...
f_audio_capture_ep_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	//printk("uac: f_audio_capture_ep_complete Copy %d bytes\n", req->actual);
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	printk("uac: audio_set_intf_req, bRequest 0x%x, w_value 0x%04x, len %d, entity %d\n",
			ctrl->bRequest, w_value, len, id);
	list_for_each_entry(cs, &audio->fu_cs, list) {
		if (cs->id == id) {
			list_for_each_entry(con, &cs->control, list) {
				if (con->type == con_sel) {
					printk("uac: set_con 111\n");
					audio->set_con = con;
					break;
				}
			}
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	struct usb_audio_control_selector *cs;
	struct usb_audio_control *con;
	printk("uac: audio_get_intf_req, bRequest 0x%x, w_value 0x%04x, len %d, entity %d\n",
			ctrl->bRequest, w_value, len, id);
	list_for_each_entry(cs, &audio->fu_cs, list) {
		if (cs->id == id) {
			list_for_each_entry(con, &cs->control, list) {
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	u8	con_sel = (w_value >> 8) & 0xFF;
	u8	cmd     = (ctrl->bRequest & 0x0F);
	//printk("uac: audio_set_endpoint_req, bRequest 0x%x, w_value 0x%04x, len %d, endp %d, epnum %d\n",
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	u8  con_sel = (w_value >> 8) & 0xFF;
	u8  cmd     = (ctrl->bRequest & 0x0F);
	printk("uac: audio_get_endpoint_req, bRequest 0x%x, w_value 0x%04x, len %d, ep %d\n",
...
f_audio_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE:
		//printk("111 USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE\n");
		value = audio_set_intf_req(f, ctrl);
		break;
	case USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE:
		//printk("222 USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE\n");
		value = audio_get_intf_req(f, ctrl);
		break;
	case USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_ENDPOINT:
		//printk("333 USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_ENDPOINT\n");
		value = audio_set_endpoint_req(f, ctrl);
		break;
	case USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_ENDPOINT:
		//printk("444 USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_ENDPOINT\n");
...
#endif
	int err = 0;
	//printk("f_audio_set_alt out_ep->desc: %p, in_ep->desc: %p\n", out_ep->desc, in_ep->desc);
	if (intf == ac_header_desc.baInterfaceNr[0]) { //MIC: 1
		if (alt == 1) {
...
#if ENABLE_PLAYBACK 
	else if (intf == ac_header_desc.baInterfaceNr[1]) { //SPK: 2
		if (alt == 1) {
...
static void f_audio_build_desc(struct f_audio *audio)
{
	...
	printk("uac: f_audio_build_desc in\n");
#if ENABLE_PLAYBACK 
	/* Set channel numbers */
	...
	speaker_as_type_i_desc.bNrChannels =
			u_audio_get_playback_channels(card);
	printk("uac: f_audio_build_desc 333, spk channel: %d, %d\n", speaker_input_terminal_desc.bNrChannels, speaker_as_type_i_desc.bNrChannels);
#endif
	microphone_input_terminal_desc.bNrChannels =
			u_audio_get_capture_channels(card);
	microphone_as_type_i_desc.bNrChannels =
			u_audio_get_capture_channels(card);
	printk("uac: f_audio_build_desc 444, mic channel: %d, %d\n", microphone_input_terminal_desc.bNrChannels, microphone_as_type_i_desc.bNrChannels);
#if ENABLE_PLAYBACK 
	/* Set sample rates */
	rate = u_audio_get_playback_rate(card);
	sam_freq = speaker_as_type_i_desc.tSamFreq[0];
	memcpy(sam_freq, &rate, 3);
	printk("uac: f_audio_build_desc 111, rate: %d\n", rate);
#endif
	rate = u_audio_get_capture_rate(card);
	sam_freq = microphone_as_type_i_desc.tSamFreq[0];
	memcpy(sam_freq, &rate, 3);
	printk("uac: f_audio_build_desc 222, rate: %d\n", rate);
...
f_audio_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	printk("uac: f_audio_bind in\n");

	f_audio_build_desc(audio);
	/* allocate instance-specific interface IDs, and patch descriptors */
	status = usb_interface_id(c, f);
	if (status < 0) {
		printk("%s: failed to allocate desc interface", __func__);
		goto fail;
	}
	ac_interface_desc.bInterfaceNumber = status;
	//audio->control_intf = status;
	uac_iad.bFirstInterface = status;

	printk("uac: f_audio_bind 111, bFirstInterface: %d, bInterfaceNumber: %d\n", uac_iad.bFirstInterface, ac_interface_desc.bInterfaceNumber);
	status = -ENOMEM;
	status = usb_interface_id(c, f);
	if (status < 0) {
		printk("%s: failed to allocate alt interface", __func__);
		goto fail;
	}
	...
	//audio->streaming_intf = status;
	printk("uac: f_audio_bind 222, baInterfaceNr0: %d\n", ac_header_desc.baInterfaceNr[0]);
	...
	printk("uac: f_audio_bind baInterfaceNr1: %d\n", ac_header_desc.baInterfaceNr[1]);
	audio->alt_intf[1] = 0;
	status = -ENODEV;
#endif
	...
	if (!ep) {
		printk("%s: failed to autoconfig in endpoint", __func__);
		goto fail;
	}
	audio->in_ep = ep;
	ep->desc = &microphone_as_ep_in_desc;
	printk("uac: f_audio_bind 111 ep: %s\n", ep->name);
	ep->driver_data = cdev;
#if ENABLE_PLAYBACK 
	status = -ENOMEM;
	ep = usb_ep_autoconfig(cdev->gadget, &speaker_as_ep_out_desc);
	if (!ep) {
		printk("%s: failed to autoconfig out endpoint", __func__);
		goto fail;
	}
	audio->out_ep = ep;
	ep->desc = &speaker_as_ep_out_desc;
	printk("uac: f_audio_bind 222 ep->desc: %p\n", audio->out_ep->desc);
	ep->driver_data = cdev;
#endif
...
int  control_selector_init(struct f_audio *audio)
{
	...
	list_add(&speaker_volume_control.list,
		 &speaker_fu_controls.control);
#endif
	//INIT_LIST_HEAD(&audio->units);
	//list_add(&feature_unit.list, &audio->units);
...
int audio_bind_config(struct usb_configuration *c)
{
	struct f_audio *audio;
	int status;
	printk("uac: audio_bind_config in\n");
	...
	// Don't lock sound card during init.
	///* set up ASLA audio devices */
	//status = gaudio_setup(&audio->card);
	//printk("uac: audio_bind_config, 111, status: %d\n", status);
	//if (status < 0)
	//	goto add_fail;
	the_card = &audio->card;
	printk("snd: audio_bind_config card: %p, %p\n", the_card, &audio->card);
	status = usb_add_function(c, &audio->card.func);
	printk("uac: audio_bind_config, 222, status: %d\n", status);
	if (status) {
		pr_err("%s: Failed to add usb audio function, err = %d",
			__func__, status);
		goto setup_fail;
	}
	printk("uac: audio_bind_config out\n");
	return status;
// Don't lock sound card during init.
//add_fail:
//	gaudio_cleanup();
