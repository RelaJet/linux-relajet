...
static DECLARE_DELAYED_WORK(pcm_close_task, gaudio_cleanup);
int pcm_close_expiry_timeout = 1 * HZ;
/*-------------------------------------------------------------------------*/
/**
...
{
	int changed;
	printk("uac: _snd_pcm_hw_param_set in\n");
	if (hw_is_mask(var)) {
	...
	printk("uac: _snd_pcm_hw_param_set out\n");
	return changed;
}
/*-------------------------------------------------------------------------*/
static inline
...
static int playback_prepare_params(struct gaudio_snd_dev *snd)
{
	...
	* RATE: 8000
	*/
	printk("uac: playback_prepare_params in\n");
	...
	if (result < 0)
		printk("uac: SNDRV_PCM_IOCTL_DROP failed: %d\n", (int)result);
	result = snd_pcm_kernel_ioctl(substream,
			SNDRV_PCM_IOCTL_HW_PARAMS, params);
	if (result < 0) {
		printk("uac: SNDRV_PCM_IOCTL_HW_PARAMS failed: %d\n", (int)result);
		kfree(params);
		return result;
	}
	result = snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_PREPARE, NULL);
	if (result < 0)
		printk("uac: Preparing playback failed: %d\n", (int)result);
	...
	kfree(params);
	printk("playback params: access %x, format %x, channels %d, rate %d\n",
		snd->access, snd->format, snd->channels, snd->rate);
	
	printk("uac: playback_prepare_params out\n");
	return 0;
}
#endif
...
static int capture_prepare_params(struct gaudio_snd_dev *snd)
{
	...
	result = snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_PREPARE,
					NULL);
	if (result < 0)
		printk("Preparing capture failed: %d\n", (int)result);
	/* Store the hardware parameters */
	snd->access = params_access(params);
	snd->format = params_format(params);
	snd->channels = params_channels(params);
	...
	if (!swparams) {
		printk("Failed to allocate sw params\n");
		return -ENOMEM;
	}
	...
	if (result < 0)
		printk("SNDRV_PCM_IOCTL_SW_PARAMS failed: %d\n", (int)result);
	kfree(swparams);
	printk("uac: capture params: access %x, format %x, channels %d, rate %d\n",
		snd->access, snd->format, snd->channels, snd->rate);
	return result;
}
/**
...
static int playback_default_hw_params(struct gaudio_snd_dev *snd)
{
	...
	*/
	...
	snd->rate = params_rate(params);
	kfree(params);
	printk("uac: playback_default_hw_params playback params: access %x, format %x, channels %d, rate %d\n",
		snd->access, snd->format, snd->channels, snd->rate);
	return 0;
}
#endif
...
static int capture_default_hw_params(struct gaudio_snd_dev *snd)
{
	...
	snd->rate = params_rate(params);
	kfree(params);
	printk("uac: capture_default_hw_params capture params: access %x, format %x, channels %d, rate %d\n",
		snd->access, snd->format, snd->channels, snd->rate);
...
static int gaudio_open_streams(void)
{
	...
	int res = 0;
	printk("uac: gaudio_open_streams in\n");
	if (!the_card) {
	...
	printk("uac: gaudio_open_streams out\n");
	return 0;
}
...
static size_t u_audio_playback(struct gaudio *card, void *buf, size_t count)
{
	...
	int err = 0;
	printk("uac: u_audio_playback in\n");
	if (!count) {
...
try_again:
	...
	set_fs(old_fs);
	printk("Done. Sent %d frames", (int)frames);
	printk("uac: u_audio_playback out\n");
	return 0;
}
#endif
...
static int gaudio_open_snd_dev(struct gaudio *card)
{
	...
	struct gaudio_snd_dev *snd;
	int res = 0;
	printk("uac: gaudio_open_snd_dev in\n");
	if (!card) {
		pr_err("%s: Card is NULL", __func__);
		return -ENODEV;
	}
	...
	snd->card = card;
	printk("uac: gaudio_open_snd_dev 111\n");
	...
	printk("uac: gaudio_open_snd_dev 222\n");
#endif
	...
	printk("uac: gaudio_open_snd_dev out\n");
	return res;
}
/**
...
static int gaudio_close_snd_dev(struct gaudio *gau)
{
	struct gaudio_snd_dev	*snd;
	pr_debug("Enter");
	printk("uac: gaudio_close_snd_dev\n");
...
{
	if (the_card) {
		printk("snd: gaudio_cleanup, close snd\n");
