...
static struct usb_endpoint_descriptor uvc_streaming_ep = {
	...
	.wMaxPacketSize		= cpu_to_le16(1024|2<<11), //Heigh bitrate, heigh bandwidth.
	.bInterval		= 1,
};
...
void uvc_function_ep0_complete1(struct usb_function *f, struct usb_request *req)
{
	struct uvc_device *uvc = to_uvc(f);
	struct v4l2_event v4l2_event;
	struct uvc_event *uvc_event = (void *)&v4l2_event.u.data;

	if (/*uvc->event_setup_out*/req->actual<=UVC_REQUEST_DATA_LEN) {
		uvc->event_setup_out = 0;

		memset(&v4l2_event, 0, sizeof(v4l2_event));
		//memset(&uvc_event->data.data,0, sizeof(uvc_event->data.data));
		v4l2_event.type = UVC_EVENT_DATA;
		uvc_event->data.length = req->actual;
...
EXPORT_SYMBOL(uvc_function_ep0_complete1);
...
uvc_function_set_alt(struct usb_function *f, unsigned interface, unsigned alt)
{
	...
	*/
	...
	case 0:
		...
		if(true == mAudioInited)
		{
			mAudioInited =false;
			schedule_delayed_work(&pcm_close_task, pcm_close_expiry_timeout);
		}
#endif
		break;
...
uvc_function_suspend(struct usb_function *f)
{
	...
	memset(&v4l2_event, 0, sizeof(v4l2_event));
	v4l2_event.type = UVC_EVENT_RESET;
	printk("usb: uvc_function_suspend UVC_EVENT_RESET\n");
	v4l2_event_queue(uvc->vdev, &v4l2_event);
...
uvc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	uvc->video.ep = ep;
	ep->driver_data = uvc;
	printk("ms: uvc_function_bind stream ep: %s\n", ep->name);
...
#if CONFIG_USB_VIDEO_GADGET_AIT
...
module_param_named(trace, uvc_gadget_trace_param, int, S_IRUGO|S_IWUSR);
